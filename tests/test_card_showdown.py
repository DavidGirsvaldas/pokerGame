...
class CardShowdownTests(unittest.TestCase):
    ...
    def test_find_winner__when_cards_missing__throws_exception(self):
        only_4_common_cards = [Card(Rank.r10, Suit.hearts), Card(Rank.Jack, Suit.diamonds),
                               Card(Rank.Queen, Suit.diamonds), Card(Rank.r2, Suit.spades)]
        player1 = self.init_test_player([Card(Rank.r2, Suit.hearts), Card(Rank.r3, Suit.clubs)])
        player2 = self.init_test_player([Card(Rank.r3, Suit.hearts), Card(Rank.r2, Suit.clubs)])
    ...
    def test_find_winner__when2players__returns_winner(self):
        common_cards = [Card(Rank.r10, Suit.hearts), Card(Rank.Jack, Suit.diamonds), Card(Rank.Queen, Suit.diamonds),
                        Card(Rank.r2, Suit.spades), Card(Rank.r5, Suit.clubs)]
        player1 = self.init_test_player([Card(Rank.Ace, Suit.hearts), Card(Rank.King, Suit.clubs)])  # has straight
        player2 = self.init_test_player([Card(Rank.Jack, Suit.hearts), Card(Rank.Jack, Suit.clubs)])  # has three of a kind
    ...
    def test_find_winner__when_player_has_higher_pair(self):
        ...
        player2 = self.init_test_player([Card(Rank.r3, Suit.hearts), Card(Rank.Jack, Suit.clubs)])  # has pair of 3s
        result = card_showdown.find_winners([player1, player2], common_cards)
        self.assertEqual(1, len(result))
        self.assertIn(player1, result)

    def test_find_winner__when3players__returns_winner(self):
        common_cards = [Card(Rank.r10, Suit.hearts), Card(Rank.Jack, Suit.diamonds), Card(Rank.Jack, Suit.diamonds),
                        Card(Rank.r2, Suit.spades), Card(Rank.r5, Suit.clubs)]
        player1 = self.init_test_player([Card(Rank.r4, Suit.hearts), Card(Rank.r3, Suit.clubs)])  # has pair of Jacks with kicker 10
        player2 = self.init_test_player([Card(Rank.Ace, Suit.hearts), Card(Rank.King, Suit.spades)])  # has pair of Jacks with kickers Ace, King
    ...
    def test_find_winner__when2players_draw__returns_both_winners(self):
        common_cards = [Card(Rank.Jack, Suit.hearts), Card(Rank.Jack, Suit.diamonds),
                        Card(Rank.Jack, Suit.clubs),
                        Card(Rank.Ace, Suit.spades), Card(Rank.King, Suit.clubs)]
        player1 = self.init_test_player([Card(Rank.r4, Suit.hearts), Card(Rank.r3, Suit.clubs)])  # hand doesnt play
        player2 = self.init_test_player([Card(Rank.Ace, Suit.hearts), Card(Rank.King, Suit.spades)])  # has top full house
